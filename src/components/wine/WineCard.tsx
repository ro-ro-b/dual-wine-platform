      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg hover:border-primary-consumer/20 transition-all duration-200">
        <div className="h-48 wine-gradient flex items-center justify-center relative">
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[wine.status] ?? "bg-slate-100 text-slate-600"}`}>
              {wine.status}
            </span>
          </div>
          {wine.anchoredAt && (
        ...
        <div className="p-5">
          <h3 className="font-semibold text-slate-900 group-hover:text-primary-consumer transition mb-1">
            {wine.wineData.name}
          </h3>
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="text-xs text-slate-500">per unit · Qty: {wine.wineData.quantity}</div>
            </div>
            <div className={`flex items-center gap-1 text-sm font-semibold ${roiPositive ? "text-green-600" : "text-red-600"}`}>
